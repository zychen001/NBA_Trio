...
<!--  <a-slider id="scatterchart-slider" v-model:value="selected" :min=1 :max=8 :marks="marks" class="scatter-slider" />-->
  <a-checkbox-group v-model:value=this.teams @change="onchange">
    <a-row>
      <a-col :flex="3">
        <a-checkbox value="Heat">Heat</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Spurs">Spurs</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Thunder">Thunder</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Cavaliers">Cavaliers</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Warriors">Warriors</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Clippers">Clippers</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Celtics">Celtics</a-checkbox>
      </a-col>
      <a-col :flex="3">
        <a-checkbox value="Lakers">Lakers</a-checkbox>
      </a-col>
    </a-row>
  </a-checkbox-group>
...
export default defineComponent ({
  ...
  data() {
    return {
      ...
      csvData: {},
      selected: 8,
      teams: ["Heat", "Spurs", "Thunder", "Cavaliers", "Warriors", "Clippers", "Celtics", "Lakers"],
      checkedData: ["Heat", "Spurs", "Thunder", "Cavaliers", "Warriors", "Clippers", "Celtics", "Lakers"],
    }
  },
  mounted() {
    this.drawChart();
  ...
  methods: {
    ...
    getStat() {
      ...
      stats["Celtics"] = [];
      stats["Lakers"] = [];
      // get data from each team's trios
      ...
      let per2 = [19.37, 20.43, 22.43, 22.1, 23.87, 22.9, 23.57, 21.2, 21.3, 19.87, 21.26, 20.17];
      let ws2 = [9.467, 9.9, 10.73, 9.73, 11.07, 9.73, 7.9, 8.07, 8.6, 7, 7.93, 7.43];
      for (let i = 0; i < 12; i++) {
        if (i < 9) {
          stats["Spurs"].push({
            per: per2[i],
      ...
      let trioTeams = this.checkedData;
      let newStats = [];
      let newStats1 = [];
      ...
      trioTeams.forEach((team, i) => {
        newStats.push(...stats[team].map(d => Object.assign(d, {color: colors[team]}))
        );
      });
    ...
    drawChart() {
      ...
      svg.append("text")
          ...
          .text(`${this.titleX}-${this.titleY} Correlations of Selected Trios`);
